...
from dataset import BilangDS, causal_mask
...
def get_ds(config):
    ...
    train_ds = BilangDS(train_ds_raw, tokenizer_src, tokenizer_tgt, config['lang_src'], config['lang_tgt'], config['seq_len'] )
    val_ds = BilangDS(val_ds_raw, tokenizer_src, tokenizer_tgt, config['lang_src'], config['lang_tgt'], config['seq_len'] )
...
def train_model(config):
    ...
    if config['preload']:
        ...
        print('preload model', model_filename)
    ...
    run_validation(model, val_dl, tokenizer_src, tokenizer_tgt, config['seq_len'], device, lambda msg: batch_iterator.write(msg), global_step, writer)
...
def run_validation(model, val_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_step, writer, num_samples=2):
    model.eval()
    ...
    with torch.no_grad():
        for batch in val_ds:
            ...
            print_msg('-'* console_w)
            print_msg(f'src: {src_txt}')
            print_msg(f'tgt: {tgt_txt}')
            print_msg(f'pred: {model_out_txt}')
